...
#Obtain MetaboLight entry(ies) inserted by user and return the IDs
    ...
    if not ID_entry: #check if string is empty
        ...
        if type(ID_entry.split()) is list :#User specified more than one MTBLS entry
            print ''
            mtbls_list = []
            for entry in ID_entry.split():
                mtbls_list.append(entry.upper())
            MTBLSid = mtbls_list
        elif 'all' in ID_entry.lower(): #DOWNLOAD MTBLS entries from FTP public server
...
#Now serious...Extract information from file *.lol
...
#       created by ISATAB_parser (*.lol)
def pp_isatab_parser(id_MTBLS=None):
#Extract information from file *.lol in a pretty print way. Primary step to get relevant information to search ontologies!!!
    ...
    if not id_MTBLS:#id_MTBLS is None -> do PP for every MTBLS entry in folder _ppisatabPARSER
        ...
        for name in os.listdir(pathISAPARSER) : #Find files already parsed by ISATAB parser
            if name.endswith("_parsed.lol") :
                parse_text(pathISAPARSER + name)##Very sensible, don't change output *.lol format
        ...
        if not os.path.isfile(pathISAPARSER + id_MTBLS + "_parsed.lol") :
            isatab_parser(id_MTBLS)
        return parse_text(pathISAPARSER + id_MTBLS + '_parsed.lol')


def parse_text(nameFile) :
#READ a file *_parsed.lol and extract relevant information:
    output = ''
    i = 0
    itemsOrg = []
    itemsOrgPart = []
    ...
    with open(nameFile) as fp :
        for line in fp :
            ...
            if ('   nodes:' not in line) :
                ...
                write_to_File(pathppISAPARSERsummary + 'pp_' + nameFile.replace('_parsed.lol','_summary.wtf'), output, 'w')
                print ', '.join(itemsAsFollows)
                write_to_File(pathppISAPARSER + 'pp_' + nameFile.replace('_parsed.lol','.wtf'), ', '.join(itemsAsFollows), 'w')
...
def isatab_parser(id_MTBLS=None):
    ...
    if not id_MTBLS :
        ...
        for name in MTBLSentries:
            ...
            fp = open(pathISAPARSER + name + '_parsed.lol', 'w')
            print >> fp, rec
            fp.close()
    else:#CASE: function called with one input parameter. USER should provide a good and existing MTBLS entry!!!!
        ...
        fp = open(pathISAPARSER + id_MTBLS + '_parsed.lol', 'w')
        print >> fp, rec
        fp.close()
...
def ext_Information_from_file(posiStart, id_MTBLS=None):
    ...
    if not id_MTBLS :
        ...
        if 'all' in opt.lower() : #PARSER all files
            ...
            opt = 'pp_MTBLS' + str(opt) + '.wtf'
        elif opt.upper().startswith('MTBLS') :
            justone = True
            opt = 'pp_' + str(opt).upper() + '.wtf'
        else:
        ...
        only_Ont = []
        with open(pathppISAPARSER + 'pp_' + id_MTBLS + '.wtf','r') as fp :
            for fpcontent in fp :
                for posiOnt in xrange(posiStart,len(fpcontent.split(',')),3) :
                    only_Ont.append(fpcontent.split(',')[posiOnt].strip())
            write_to_File(pathFile + id_MTBLS + '.wtf', ';'.join(only_Ont), 'w')
        return only_Ont
...
def get_TimeRemaining(actEntry, totEntries, MTBLSentry):
    print('\tProcessing: ' + str(actEntry) + ' of ' + str(totEntries))
...
#    from html import HTML
    ...
    os.system('clear')
    print "Calculating the quantitative score...NOT YET!"
    counter = 0
    ontologies = [] #Store the ontologies extracted!!!
    organism = []

    db_onto = REST_getAllOntologiesBioportal()#get updated database with all ontologies available in bioporta
    #h = HTML()
    #fp = open('htht.html','w')
    #for elem in db_onto:
    #    h.p(elem['acr'])
    ...
    for MTBLSentry in MTBLSentries :
        ...
        if os.path.exists(pathppISAPARSERsummary + "pp_" + MTBLSentry + "_summary.wtf") :
            os.remove(pathppISAPARSERsummary + "pp_" + MTBLSentry + "_summary.wtf")
        ...
        print "\t\tTEST: PARSING"
        parsingScore = REST_SCOREparsing(MTBLSentry)
        print "\t\t\tPARSE_SCORE: " + str(parsingScore) + " of 1(max_value)"

        print "\t\tTEST: LEXICAL"
        score, relScore = REST_SCOREOntoLexic(MTBLSentry, db_onto)
        ...
        for org, ontol  in zip(organism, ontologies) :
            if org != 'NONE' and ontol != 'NONE':
                ...
                REST_get_Recommender(MTBLSentry, org, ontol)
            else:
...
def REST_recommender_TreeDepth(node):
#get the tree depth based on a given node
    depth = 1
    print "\t\t\t\tCalculating Tree Depth..."

    print "\t\t\t\t\t 0  " + REST_recommender_GetPrefLabel(node)
    ...
    while(1):
        ...
        if REST_recommender_HasParent(get_json(node["links"]["ancestors"])[0]) == "No":
            print "\t\t\t\t\t " + str(depth) + "  " + REST_recommender_GetPrefLabel(get_json(node["links"]["ancestors"])[0])
            return depth
...
#Search if ontologies/REF have known values
    fp = open(pathppISAPARSER + "pp_" + id_MTBLS + ".wtf", 'r')
    ...
    for i in xrange(1,len(line),3) :
        ...
        for acr in db_onto:#Compare with all ontologies in bioportal
            ...
            if found_ont or found_ref : #OR
                ...
                if found_ont == 1: #Found_ONT AND notFound_REF
                    score += score_nfREF_fONT
                else: #notFound_ONT AND Found_REF
                    score += score_fREF_nfONT
                break;

    return score, str(3*relFound/(len(line))*100)

def REST_SCOREparsing(id_MTBLS):
...
#TEST:get self link based on term and ontology provided -> return NONE is link doesn't found
    ...
    for ontoID in recom:
        ...
        if idOntology.find(ontology) != -1 : #Found relation
    ...
    #print 'EQUALS\n' + str(len(SelfsLinkEqual))
    #for ele in SelfsLinkEqual :
    #    print ele
    #
    #print '\nCONTAINS\n' + str(len(SelfsLinkContains) )
    #for ele in SelfsLinkContains :
    #    print ele

    #return SelfsLinkEqual if len(SelfsLinkEqual) != 0  else SelfsLinkContains
    return SelfsLinkEqual, SelfsLinkContains


def print_recommendations(MTBLSentry, recom):
#Given a specific term, extract information to a file in 'OUTPUT/_ppisatabPARSER/[MTBLSID].btw
    #
    ...
    for result in recom:
        ...
        if turn == 1:
            ...
            tree_depth =  REST_recommender_TreeDepth(class_details)
            ...
            #    print >>fp, "\tprefLabel: " + class_details["prefLabel"]
            #    print >>fp, "\tid: " + class_details["@id"]
            #    print >>fp, "\thas child: " + has_child
            #    print >>fp, "\thas parent: " + has_parent
            #    print >>fp, "\ttree depth: " + str(tree_depth)
            #    print >>fp, "Recommended ontologies"
            #
